...
def plot_results(train_results, val_results, model):
  ...
  for i, ((train_epoch, train_data), (val_epoch, val_data)) in enumerate(zip(train_results.items(), val_results.items())):
    ...
    plot_losses(train_batch_losses, val_batch_losses, title=f"Batch Losses for {model.name} at Epoch {train_epoch + 1}", save_name=f"{model.name}_epoch_{train_epoch + 1}_losses", xlabel='Step', ylabel='Loss')
    
    
  plot_losses(("Train Loss", all_train_losses, 'blue'), ("Val Loss", all_val_losses, 'red'), title=f"All Losses for {model.name}", save_name=f"{model.name}_all_losses", xlabel='Step', ylabel='Loss', mark_epochs=True, epoch_steps=epoch_steps)